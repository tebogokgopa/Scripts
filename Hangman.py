...
def get_num_attempts():
    ...
    while True:
        num_attempts = input('How many correct attempts do you want? [1-25] ')
        try:
...
def play_hangman():
    ...
    """
    #Let the player specify difficulty
    ...
    while attempts_remaining > 0 and not word_solved:
        ...
        print('Previous gusses: {0}'.format(''.join(wrong_letters)))
        ...
        if next_letter in word:
            #Guessed correctly
            print('{0} is in word!'.format(next_letter))
        ...
        #Decrement  number of attempts left and append guess to wrong guesse
            attempts_remaining -= 1
            wrong_letters.append(next_letter)

        #Check if word is completetly solve
